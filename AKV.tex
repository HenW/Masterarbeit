...
\begin{acronym}[Bash]
 ...
 \acro{ahb}[AHB]{Advanced High-Performance Bus}
  \acro{apb}[APB]{Advanced Peripherie Bus}
...
\acro{dram}[DRAM]{Dynamic Random Access Memory}
 \acro{mips}[MIPS]{Millions of Instructions per second}
\acro{dmips}[DMIPS]{Dhrystone Millions of Instructions per second}
 \acro{ipc}[IPC]{Instruction per Cycle}
\acro{mflops}[MFLOPS]{Millions of Floating-point Operations per second}
\acro{eembc}[EEMBC]{Embedded Microprocessor Benchmark Consortium}
\acro{mcu}[MCU]{Microcontroller}
\acro{crc}[CRC]{Cyclic Redundancy Check}
\acro{spec}[SPEC]{Standard Performance Evaluation Corp.}
\acro{lmb}[LMB]{Local Memory Bus}
\acro{bs}[BS]{Barrel Shifter Unit}
\acro{io}[IO]{Input-Output}
\acro{bufg}[BUFG]{Global Clock Buffer}
\acro{lutram}[LUTRAM]{Look-Up-Table Random Access Memory}
\acro{ff}[FF]{Flip Flop}
\acro{loaf}[LOAF]{Linux on A Floppy}
\acro{cu}[CU]{Control Unit}
\acro{alu}[ALU]{Arithmetic Logic Unit}
\acro{assp}[ASSP]{Application-Specific Standard Product}
\end{acronym}
